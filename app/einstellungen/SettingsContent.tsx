...
function SectionCard({
  ...
  return (
    <div id={id} className="bg-white scroll-mt-24" style={{ border: "1px solid #E5E7EB" }}>
      <button onClick={onToggle} className="w-full flex items-center gap-4 px-6 py-5 text-left">
        <div className="w-10 h-10 flex-shrink-0 flex items-center justify-center" style={{ background: "rgba(232,168,56,0.1)" }}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[13px]" style={{ color: "#6B7280" }}>{desc}</p>
        </div>
        {open ? <ChevronDown className="w-5 h-5 text-muted" /> : <ChevronRight className="w-5 h-5 text-muted" />}
      </button>
      {open && <div className="px-6 pb-6 pt-1" style={{ borderTop: "1px solid #F3F4F6" }}>{children}</div>}
    </div>
  );
}
    ...
    <div className="pt-3 space-y-8">
      ...
      <div className="pt-6" style={{ borderTop: "1px solid #F3F4F6" }}>
        ...
        {activity === null ? (
          ...
          <p className="text-sm" style={{ color: "#6B7280" }}>{t("security.noActivity")}</p>
        ) : (
          <ul className="space-y-2">
            {activity.map((a, i) => (
              <li key={i} className="flex items-center justify-between text-[13px] px-3 py-2 gap-3" style={{ background: "#F8F7F4" }}>
                <span className="text-primary truncate">{label[a.action] ?? a.action}</span>
                <span className="flex-shrink-0" style={{ color: "#6B7280" }}>{new Date(a.at).toLocaleString(lang)}</span>
              </li>
            ))}
          </ul>
        )}
        <button onClick={onLogout} className="mt-4 inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold" style={{ border: "1.5px solid #E5E7EB", color: "rgba(26,26,46,0.7)" }}>
          <LogOut className="w-4 h-4" />{t("btn.logoutAll")}
        </button>
        <p className="text-[11px] mt-2" style={{ color: "rgba(107,114,128,0.7)" }}>{t("security.logoutAllHint")}</p>
      </div>
    </div>
  );
}
      ...
      <div className="space-y-2">
        {faqs.map((f, i) => (
          <div key={i} style={{ border: "1px solid #E5E7EB" }}>
            ...
            {open === i && <p className="px-4 pb-3 text-[13px] leading-relaxed" style={{ color: "#6B7280" }}>{f.a}</p>}
          </div>
        ))}
      </div>
...
function DataSection({ token, t, onDeleted }: { token: string; t: T; onDeleted: () => void }) {
  ...
  return (
    <div className="pt-3 space-y-6">
      <div>
        <p className="text-sm mb-3" style={{ color: "#6B7280" }}>{t("danger.exportHint")}</p>
        <button onClick={doExport} disabled={exporting} className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold" style={{ border: "1.5px solid #E5E7EB", color: "rgba(26,26,46,0.75)" }}>
      ...
      <div className="pt-5" style={{ borderTop: "1px solid #F3F4F6" }}>
        <p className="text-sm mb-3" style={{ color: "#6B7280" }}>{t("danger.deleteHint")}</p>
        {!confirmDel ? (
          ...
          <div className="space-y-3 p-4" style={{ background: "rgba(239,68,68,0.04)", border: "1px solid rgba(239,68,68,0.25)" }}>
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => { setConfirmDel(false); setDelPw(""); setDelErr(null); }} className="text-sm" style={{ color: "#6B7280" }}>{t("btn.cancel")}</button>
            </div>
          </div>
        )}
      </div>
    ...
    <div className="min-h-screen" style={{ background: "#F8F7F4", fontFamily: "var(--font-sans)" }}>
      ...
      <div className="max-w-3xl mx-auto px-6 -mt-8 pb-16 space-y-4">
        {shown !== null && shown.size === 0 && (
          <div className="bg-white px-6 py-10 text-center" style={{ border: "1px solid #E5E7EB" }}>
            <p className="text-sm" style={{ color: "#6B7280" }}>{t("search.none")} &bdquo;{search}&ldquo;</p>
          </div>
        )}
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  {LANGUAGES.map((l) => (
                    <button key={l.code} onClick={() => setLang(l.code)} className="flex items-center gap-3 px-4 py-3 text-sm font-medium transition-all"
                      style={{ border: `1.5px solid ${lang === l.code ? "#E8A838" : "#E5E7EB"}`, background: lang === l.code ? "rgba(232,168,56,0.06)" : "white", color: lang === l.code ? "#1A1A2E" : "rgba(26,26,46,0.6)" }}>
                      <span className="text-lg">{l.flag}</span>{l.label}
                      {lang === l.code && <Check className="w-4 h-4 ml-auto" style={{ color: "#E8A838" }} strokeWidth={3} />}
                    </button>
                  ))}
                </div>
              )}
              ...
              {id === "legal" && (
                <div className="pt-2 grid gap-2">
                  {[
                    ...
                    <Link key={x.href} href={x.href} target="_blank" className="flex items-center justify-between px-4 py-3 text-sm hover:bg-surface transition-colors" style={{ border: "1px solid #E5E7EB", color: "rgba(26,26,46,0.75)" }}>
        ...
        <p className="text-center text-[11px] pt-6" style={{ color: "rgba(26,26,46,0.35)" }}>🇩🇪 {t("madeIn")}</p>
      </div>
    </div>
  );
}
